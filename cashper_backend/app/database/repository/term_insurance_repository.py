...
class TermInsuranceRepository:
    ...
    def get_inquiry_collection(self):
        ...
        return self.db["term_insurance_inquiries"]
    
    def get_application_collection(self):
        """Lazy initialization for application collection"""
        if self.db is None:
            self.db = get_database()
        return self.db["term_insurance_applications"]

    # ==================== INQUIRY OPERATIONS ====================
    
    def create_inquiry(self, inquiry_data: TermInsuranceInquiryInDB) -> str:
        """Create a new term insurance inquiry"""
    ...
    def create_application(self, application_data: TermInsuranceApplicationInDB) -> str:
        """Create a new term insurance application"""
    ...
    def get_all_applications(self, skip: int = 0, limit: int = 100) -> List[dict]:
        ...
        collection = self.get_application_collection()
        cursor = collection.find().skip(skip).limit(limit).sort("submittedAt", -1)
    ...
    def get_applications_by_status(self, status: ApplicationStatus) -> List[dict]:
        ...
        collection = self.get_application_collection()
        cursor = collection.find({"status": status}).sort("submittedAt", -1)
        applications = list(cursor)
        for application in applications:
            application["_id"] = str(application["_id"])
        return applications
    ...
    def get_applications_by_email(self, email: str) -> List[dict]:
        ...
        collection = self.get_application_collection()
        cursor = collection.find({"email": email}).sort("submittedAt", -1)
        applications = list(cursor)
        for application in applications:
            application["_id"] = str(application["_id"])
        return applications
...
term_insurance_repository = TermInsuranceRepository()
