...
def get_collections():
    ...
    db = get_database()
    return {
        'get_in_touch': db["personal_loan_get_in_touch"],
        'applications': db["personal_loan_applications"],
        'eligibility': db["personal_loan_eligibility_criteria"]
    }
...
def seed_eligibility_criteria():
    ...
    if collection.count_documents({}) == 0:
        criteria = [
            {"label": "Age", "value": "21 to 60 years", "order": 1, "created_at": datetime.now()},
            {"label": "Employment", "value": "Salaried/Self-employed", "order": 2, "created_at": datetime.now()},
            {"label": "Minimum Income", "value": "₹20,000 per month", "order": 3, "created_at": datetime.now()},
            {"label": "Credit Score", "value": "700 and above (preferred)", "order": 4, "created_at": datetime.now()},
            {"label": "Work Experience", "value": "Minimum 1 year", "order": 5, "created_at": datetime.now()},
            {"label": "Nationality", "value": "Indian Resident", "order": 6, "created_at": datetime.now()},
        ]
